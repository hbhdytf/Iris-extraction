...
void gaussian_smooth(Mat& image, Mat& res, double sigma, int size)
{
    ...
    for (int j = o; j < kh + o; j++, pos+=off)
    {
        ...
        {
            *pos = exp(-(i * i + j * j) / (2 * sigma2)) / (2 * pi * sigma2);
        }
    }
    ...
    for (int r = min_r; r <= max_r; r++)
    {
        ...
        for (int j = 0; j < ny; j++)
        {
            for (int i = 0; i < nx; i++)
            {
                ...
                if (v)
                {
                    HoughCircle c = {min_cx + i, min_cy + j, r, (int)(2 * pi * r), v, (int)(2 * pi * r * 255)};
...
int main(int argc, char *argv[])
{
	...
	try {
    	...
    	if (mode == MODE_MAIN){
			...
			cmdCheckOpts(cmd,"-i|-o|-m|-s|-e|-q|-t|-po|-bm|-sr|-lt");
			...
			for (vector<string>::iterator inFile = files.begin(); inFile != files.end(); ++inFile, timing.progress++){
				...
				adjust_luminance(temp, min_ir * min_ir * pi, width * height * 0.01);
				...
				polar2Cart(cont, pupilCart, px, py);
				cont.setTo(ir);
				polar2Cart(cont, irisCart, ix, iy);
