 ...
 */
void featureExtract(Mat& code, Mat& codeMask, const Mat& texture, const Mat& textureMask, const int M = 5, const int N = 10)
{
    ...
    for (int k = 0; k < nscales; k++)
    {
    	for (int y = 0; y < N; y++)
    	{
    		for (int x = 0; x < w; x++)
    		{
    			...
    			if (H1)	codeData[0] |= (1 << bitpos);
    			if (H2) codeData[rowlen] |= (1 << bitpos);
    			bitpos++;
    		}
    		codeData += rowlen;
    	...
    	if (mode == MODE_MAIN){
			...
			for (vector<string>::iterator inFile = files.begin(); inFile != files.end(); ++inFile, timing.progress++){
				...
				if (!imaskFiles.empty()) {
					...
					CV_Assert(mask.size() == Size(512,64));
				}
