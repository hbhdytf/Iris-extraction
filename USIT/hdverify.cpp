 ...
 */
double minHD(const Mat& a, const Mat& b, const unsigned int start8, const unsigned int stop8, const int minShifts, const int maxShifts, const Mat aMask, const Mat bMask = Mat()){
	double result = 0;
	if (!aMask.empty() && !bMask.empty()){
		...
		for (int s=minShifts; s<=maxShifts; s++){
 ...
 * bMask: mask for second iris code
 */
double maxHD(const Mat& a, const Mat& b, const unsigned int start8, const unsigned int stop8, const int minShifts, const int maxShifts, const Mat aMask, const Mat bMask = Mat()){
	double result = 0;
	if (!aMask.empty() && !bMask.empty()){
		Mat mask(b.rows,b.cols,CV_8UC1);
		Mat zero(b.rows,b.cols,CV_8UC1);
 ...
 * bMask: mask for second iris code
 */
double ssf(const Mat& a, const Mat& b, const unsigned int start8, const unsigned int stop8, const int minShifts, const int maxShifts, const Mat aMask, const Mat bMask = Mat()){
	double result = 0;
	if (!aMask.empty() && !bMask.empty()){
		Mat mask(b.rows,b.cols,CV_8UC1);
		Mat zero(b.rows,b.cols,CV_8UC1);
 ...
 * shifts: number of shifts
 */
double compare(const vector<Mat>& imgSmpl, const Mat& imgRef, const unsigned int from, const unsigned int bitStop, const int minShifts, const int maxShifts, int alg, const vector<Mat>& maskSmpl, const Mat& maskRef, bool shiftedfiles){
	if (alg == ALG_MINHD) {
		if (shiftedfiles) return minHD(imgSmpl,imgRef,from,bitStop, maskSmpl, maskRef); else return minHD(imgSmpl[0],imgRef,from,bitStop,minShifts, maxShifts, (maskSmpl.size() > 0) ? maskSmpl[0] : Mat(), maskRef);
	}
	else if (alg == ALG_MAXHD) {
		if (shiftedfiles) return maxHD(imgSmpl,imgRef,from,bitStop, maskSmpl, maskRef); else return maxHD(imgSmpl[0],imgRef,from,bitStop,minShifts, maxShifts, (maskSmpl.size() > 0) ? maskSmpl[0] : Mat(), maskRef);
	}
	else if (shiftedfiles) return ssf(imgSmpl,imgRef,from,bitStop,maskSmpl, maskRef); else return ssf(imgSmpl[0],imgRef,from,bitStop,minShifts, maxShifts, (maskSmpl.size() > 0) ? maskSmpl[0] : Mat(), maskRef);
}
...
int main(int argc, char *argv[])
{
	...
	try {
		...
		if (mode == MODE_MAIN){
			...
			cmdCheckOpts(cmd,"-i|-s|-m|-c|-a|-n|-b|-o|-r|-d|-q|-t");
				...
				for (map<string, vector<string> >::iterator it = userTemplates.begin(); it != userTemplates.end(); it++){
					for (vector<string>::iterator itSample = it->second.begin(), itEnd = it->second.end(); itSample != itEnd; itSample++){
						...
						for (itRef++; itRef != itEnd; itRef++, timing.progress++){
							...
							double score = compare(imgSmpl,imgRef,from,bitStop,minShifts,maxShifts,alg,maskSmpl,maskRef,shiftedfiles);
