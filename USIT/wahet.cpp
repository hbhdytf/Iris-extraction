...
double boundaryEnergy(const Mat& src, const Mat& cartBoundary){
	...
	int width = src.cols;
	int height = src.rows;
	MatConstIterator_<float> b = cartBoundary.begin<float>();
	MatConstIterator_<float> e = cartBoundary.end<float>();
	while(b!=e){
...
int main(int argc, char *argv[])
{
	...
	try {
    	...
    	if (mode == MODE_MAIN){
			...
			cmdCheckOpts(cmd,"-i|-o|-s|-e|-q|-t|-rm|-rr|-em|-gr|-ic|-po|-fb|-ep|-ib|-ob|-sr|-bm|-lt");
			...
			for (vector<string>::iterator inFile = files.begin(); inFile != files.end(); ++inFile, timing.progress++){
				...
				inpaint(img,mask,img,10.,CV_INPAINT_NS);
