...
int main(int argc, char *argv[])
{
	...
	try {
		...
		if (mode == MODE_MAIN){
			...
			cmdCheckOpts(cmd,"-i|-m|-s|-a|-n|-o|-q|-t");
			...
			for (vector<string>::iterator infileSmpl = filesSmpl.begin(); infileSmpl != filesSmpl.end(); ++infileSmpl){
				...
				for (vector<string>::iterator infileRef = filesRef.begin(); infileRef != filesRef.end(); ++infileRef, timing.progress++){
					...
					double score = (alg == ALG_MINHD) ? (shiftedfiles) ? minHD(imgSmpl,imgRef,from,bitStop, maskSmpl, maskRef) : minHD(imgSmpl[0],imgRef,from,bitStop,minShifts, maxShifts, (maskSmpl.size() > 0) ? maskSmpl[0] : Mat(), maskRef) :
								(alg == ALG_MAXHD) ? (shiftedfiles) ? maxHD(imgSmpl,imgRef,from,bitStop, maskSmpl, maskRef) : maxHD(imgSmpl[0],imgRef,from,bitStop,minShifts, maxShifts, (maskSmpl.size() > 0) ? maskSmpl[0] : Mat(), maskRef) :
								(shiftedfiles) ? ssf(imgSmpl,imgRef,from,bitStop,maskSmpl, maskRef) : ssf(imgSmpl[0],imgRef,from,bitStop,minShifts, maxShifts, (maskSmpl.size() > 0) ? maskSmpl[0] : Mat(), maskRef);
					if (!quiet) printf("hd(%s,%s) = %f\n",(*infileSmpl).c_str(), (*infileRef).c_str(), score);
